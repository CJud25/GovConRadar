...
DATA_DICTIONARY = [
    ...
    ("fact_recompete_candidates", "displacement_signal_count", "ESTIMATE — how many of the readable displacement signals fired (bridge / termination / deobligation / lapsed_no_successor / sole_offer / size_shift); empty = Unknown (insufficient coverage). A categorical lane — NEVER blended into pursuit_score or any tier.", "int", True),
    ("fact_recompete_candidates", "displacement_signals_read", "Coverage fact — how many of the 6 displacement inputs were readable on this record", "int", False),
    ("fact_recompete_candidates", "displacement_signals", "ESTIMATE — '+'-joined slugs of the fired displacement signals ('none' when none fired); empty = Unknown", "string", True),
    ("fact_recompete_candidates", "displacement_unread", "Coverage fact — '+'-joined slugs of the displacement inputs that could NOT be read ('none' when all 6 read)", "string", False),
    ("fact_recompete_candidates", "displacement_band", "ESTIMATE — multiple_signals / single_signal / none_observed / not_applicable (insufficient coverage)", "string", True),
    ("fact_recompete_candidates", "displacement_basis", "Basis for the displacement lane: observed / insufficient (fewer than min_signals_read readable inputs — Unknown, never guessed)", "string", True),
    ...
    ("dim_agency", "concentration_top_share", "Derived from facts — the top incumbent's share of the component's attributed (UEI-known) expiring obligated dollars within the REPORTABLE (Data-Gap-excluded) set — the denominator excludes dollars with no incumbent UEI (bounded by the coverage gate); a ratio of two sums of published obligated-dollar facts (like hhi_concentration); empty = Unknown (see concentration_basis). NOT market share, market power, or contestability.", "float", False),
    ("dim_agency", "concentration_n_ueis", "Coverage fact — distinct positive-net incumbent UEIs in the component's reportable set (always published, even when the share is Unknown)", "int", False),
    ("dim_agency", "concentration_basis", "Basis for the concentration read: observed / insufficient (thin market, low UEI coverage, no positive dollars, or no reportable rows — Unknown, never guessed)", "string", True),
    ("dim_agency", "concentration_reason", "Named refusal reason when concentration_basis is insufficient; empty when observed", "string", True),
