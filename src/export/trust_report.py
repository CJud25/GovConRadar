...
from labels.ingest import load_link_labels
...
def _scan_snapshots(
    snapshots_root: Path | None, comparable_since: str, current_scorer: str
) -> tuple[dict[str, frozenset[str]], pd.DataFrame, str]:
    ...
    the earliest comparable snapshot date)."""
    ...
    if snapshots_root is None or not Path(snapshots_root).is_dir():
        return ids_by_date, pd.DataFrame(columns=["candidate_id", "notice_id", "link_confidence", "posted_date"]), ""
    ...
    linked = (
        ...
        if linked_parts
        else pd.DataFrame(columns=["candidate_id", "notice_id", "link_confidence", "posted_date"])
    )
    earliest = min(ids_by_date) if ids_by_date else ""
    return ids_by_date, linked, earliest
...
def build_trust_metrics_report(
    ...
    ids_by_date, linked, earliest = _scan_snapshots(snapshots_root, comparable_since, current_scorer)
